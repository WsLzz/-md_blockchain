...
有对多线程并行调度感兴趣的，参考另一个项目 https://gitee.com/tianyalei/asyncTool  该并发框架支持任意的多线程并行、串行、阻塞、依赖、回调，可以任意组合各线程的执行顺序，还带全链路回调。该项目在京东app后台正在试用，有海量用户、高并发等各种复杂极端场景。是作为Java程序员学习多线程的不可多得的好项目。
